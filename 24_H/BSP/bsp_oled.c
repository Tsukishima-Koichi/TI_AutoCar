...
/***************************************************************************************
  ...
  * 
  * 程序名称：				0.96寸OLED显示屏驱动程序（4针脚IIC接口）
  * 程序创建时间：			2023.10.24
  * 当前程序版本：			V2.0
  * 当前版本发布时间：		2024.10.20
  * 
...
void OLED_IIC_Send_Ack(unsigned char ack)
{
    IIC_Send_Ack(ack);
}
...
{
	OLED_IIC_Start();				//IIC起始

	OLED_IIC_SendByte(0x78);		//发送OLED的IIC从机地址
    if(OLED_IIC_WaitAck() == 1)return;

	OLED_IIC_SendByte(0x00);		//控制字节，给0x00，表示即将写命令
    if(OLED_IIC_WaitAck() == 1)return;

	OLED_IIC_SendByte(Command);		//写入指定的命令
    if(OLED_IIC_WaitAck() == 1)return;
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    ...
    OLED_IIC_SendByte(0x40);		//控制字节，给0x40，表示即将写数据
    if(OLED_IIC_WaitAck() == 1)return;

	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		OLED_IIC_SendByte(Data[i]);	//依次发送Data的每一个数据
        if(OLED_IIC_WaitAck() == 1)return;
    }
